...
import { toggleExternalLessonReviewAction } from "@/modules/study-hub/actions";

export function ExternalLessonReviewToggle({
  lessonId,
  initialMarked,
}: {
  lessonId: string;
  ...
  function toggle() {
    ...
    startTransition(async () => {
      const result = await toggleExternalLessonReviewAction(lessonId);
      if (result?.error) {
